...
use semantica_core::domain::{JobState, JobType, JobPayload};
...
async fn test_enqueue_100_files() {
    ...
    let service = DevTaskService::new(job_repo.clone());
    ...
    let jobs = job_repo.list().await.unwrap();
...
async fn test_persistence_after_restart() {
    ...
    // Phase 2: Restart daemon and verify jobs
    {
        ...
        let jobs = job_repo.list().await.unwrap();
        assert_eq!(jobs.len(), 10, "All jobs should be restored");
...
/// DoD 3: No SQLITE_BUSY errors under load
#[tokio::test]
async fn test_concurrent_enqueue_no_busy_error() {
    ...
    let time_provider = Arc::new(SystemTimeProvider);
    let job_repo = Arc::new(SqliteJobRepository::new(pool, time_provider));
    let service = Arc::new(DevTaskService::new(job_repo.clone()));

    // Spawn 10 concurrent tasks, each enqueuing 10 jobs
    let mut handles = vec![];
    ...
    for task_id in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for i in 0..10 {
                ...
                // This should NOT return SQLITE_BUSY error
    ...
    // Verify total count
    let jobs = job_repo.list().await.unwrap();
    assert_eq!(jobs.len(), 100, "All concurrent enqueues should succeed");

    println!("✅ DoD 3: No SQLITE_BUSY errors under concurrent load");
}
...
async fn test_cancel_api_functional() {
    ...
    // Verify cancellation
    let cancelled_job = job_repo.find_by_id(&job_id).await.unwrap().unwrap();
    assert_eq!(cancelled_job.state, JobState::Cancelled);

    println!("✅ DoD 5: dev.cancel API is functional");
}
